		...
		>
			...
			<div class="relative w-full max-w-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl">
				<div class="p-6 border-b border-gray-100 dark:border-gray-700">
					<h2 id="delete-bank-title" class="text-lg font-semibold text-gray-900 dark:text-white">
						Confirmar exclusão de banco
					</h2>
					<p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
						Esta ação é irreversível. O banco
						<strong class="text-gray-700 dark:text-gray-200">{{ bank?.name }}</strong>
				...
				<div class="p-6 space-y-4">
					...
					<div>
						<label for="delete-bank-confirmation" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
							Confirmação de segurança
						</label>
						<input
							id="delete-bank-confirmation"
				...
				<div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-2">
					...
					<AppButton variant="danger" :disabled="!isConfirmationValid || loading" @click="confirm">
						{{ loading ? 'A excluir...' : 'Excluir Banco' }}
					</AppButton>
				</div>
			</div>
		</div>
