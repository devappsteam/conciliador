...
const emit = defineEmits<{
	...
	(event: 'update-field', field: keyof BankFormValues, value: BankFormValues[keyof BankFormValues]): void
}>()
</script>
	...
	<form :id="props.formId" class="space-y-4 p-6" novalidate @submit.prevent="emit('submit')">
		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
			<div class="md:col-span-2">
				<label for="bank-name" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
					Nome
				</label>
				<input
					id="bank-name"
					...
					:aria-describedby="fieldErrors.name ? 'bank-name-error' : undefined"
				/>
				<p v-if="fieldErrors.name" id="bank-name-error" class="mt-1.5 text-xs font-medium text-red-600 dark:text-red-400">
					{{ fieldErrors.name }}
				</p>
			</div>

			<div>
				<label for="bank-code" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
					Código
				</label>
				<input
					id="bank-code"
					...
					:aria-describedby="fieldErrors.code ? 'bank-code-error' : undefined"
				/>
				<p v-if="fieldErrors.code" id="bank-code-error" class="mt-1.5 text-xs font-medium text-red-600 dark:text-red-400">
					{{ fieldErrors.code }}
				</p>
			</div>
			...
			<div class="flex items-center gap-3">
				...
				<label for="bank-status" class="text-sm font-medium text-gray-700 dark:text-gray-300">
					{{ values.status ? 'Ativo' : 'Inativo' }}
				</label>
			</div>

			<div class="md:col-span-2">
				<label for="bank-logo" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
					Logo
				</label>
