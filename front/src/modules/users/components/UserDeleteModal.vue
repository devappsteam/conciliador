...
const isConfirmationValid = computed(() => {
	...
	const normalizedInput = confirmationInput.value.trim().toLowerCase()
	return normalizedInput === expectedKeyword
})
...
<template>
	<Teleport to="body">
		...
		>
			...
			<div
				class="relative w-full max-w-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl"
			>
				<div class="p-6 border-b border-gray-100 dark:border-gray-700">
					<h2 id="delete-user-title" class="text-lg font-semibold text-gray-900 dark:text-white">
				...
				<div
					class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-2"
				>
					...
					<AppButton variant="danger" :disabled="!isConfirmationValid || loading" @click="confirm">
						{{ loading ? 'A excluir...' : 'Excluir Usuário' }}
					</AppButton>
				</div>
			</div>
		</div>
